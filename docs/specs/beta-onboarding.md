...
28. As a user selecting a Refinement Mode, I want only the active mode to have the selected appearance, so that background and border emphasis agree with its selection mark.
...
- Refinement Mode card fill, border, and selection mark derive from the active mode. Keyboard focus and an open Custom Mode editor do not create a second selected appearance. DeepSeek-dependent modes require a stored DeepSeek Key; their availability does not require a successful connection check. Copy must describe that actual requirement. The explicit successful-connection gate applies to Doubao onboarding completion.
...
- The proposed acceptance seams are the existing App Scenario and native AppKit UI executables, plus the packaging/mount contract for installation. No new product testing seam is required. User confirmation of this seam choice and the recording-ticket breakdown is the final publication step.
