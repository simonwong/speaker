...
- Audio capture attempts to enable Apple voice processing before the engine starts. If voice processing cannot be enabled, capture falls back to the existing raw path. The resulting audio is converted to 16 kHz, 16-bit, mono PCM in memory and placed in a bounded stream. It is never persisted as a normal application artifact.
