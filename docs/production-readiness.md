...
- [ ] 建立可复现 CI 构建、自动测试、签名、公证、校验和及制品留存。（未签名的测试/严格编译/Release bundle CI 已落地；受 `production` Environment 保护的手动 workflow 也已接通临时 Keychain、Developer ID、`notarytool` API Key、Sparkle 私钥、`scripts/distribute` 和 90 天受保护制品留存。正式流程会生成 hash-bound evidence archive，包含 dSYM、BuildManifest、两次公证 submission/log 与 toolchain 信息；仍需配置正式身份并取得一次真实成功运行证据。）
