...
`SettingsNavigationModel` is the single page-selection source for the settings sections. It separates ordinary top-of-page presentation from one-shot requests to reveal a specific section. About is a separate top-level main-window tab. The five main-window tabs are a native segmented control in a transparent title bar (`MainWindowTabs`), not a tab view: the selected page fills the window on one shared ground and fades out below the title bar when scrolled. `MenuBarCommandRouter` selects the intended destination before activating Speaker; the ordinary Settings command returns to the page top.
