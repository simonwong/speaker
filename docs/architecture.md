...
Every main-window tab and the onboarding window share one adaptive surface language from `SpeakerSurfaces` and `SettingsControlStyles`. macOS 26 uses Liquid Glass for cards (`speakerCard`, which `SettingsCard` composes), for the one field surface shared by text fields, secure fields, prompt editors, and the History search bar (`speakerField`), for mode choices, and for native glass button styles. Older systems use the system material with bordered buttons; Reduce Transparency selects opaque card paper and inset fields. `AdaptiveGlassSurfacePolicy` makes that choice once, through `EnvironmentValues.adaptiveGlassSurfaceStyle`. Native menus and switches keep system behavior. Motion follows one strong ease-out curve in short rungs from `SpeakerMotion`, which the HUD panel's AppKit fades share; the Overview charts play their entrance once per launch, on the first usage snapshot, and later only retarget. Reduce Motion keeps fades and drops movement. Input focus and Increase Contrast remain visible, and styling preserves native editing and accessibility actions.

`SettingsNavigationModel` is the single page-selection source for the settings sections. It separates ordinary top-of-page presentation from one-shot requests to reveal a specific section. About is a separate top-level main-window tab. The five main-window tabs are a tab bar in a transparent title bar (`MainWindowTabs`, `MainWindowTabBar`), not a tab view or a segmented control, so no hairline separates unselected pages; assistive technologies still read it as a segmented picker. The selected page fills the window on one shared ground and fades out below the title bar when scrolled. `MenuBarCommandRouter` selects the intended destination before activating Speaker; the ordinary Settings command returns to the page top.
...
`SpeechRecognitionProviderSettings` remembers recognition profiles independently, including the Speech Recognition Method. `VoiceInputConfigurationController` includes the selected profile in its capture snapshot, and `CredentialedSpeechTranscriber` routes the same contextual transcription seam to Doubao or the selected OpenAI/Qwen method. Complete-recording recognition waits on the session-owned `AudioCaptureCompletionGate` before uploading an in-memory WAV through HTTP. Streaming recognition sends PCM over a realtime WebSocket, receives events concurrently, and waits on that same gate before its final manual audio commit. The OpenAI streaming path resamples 16 kHz PCM to 24 kHz in bounded memory. Automatic turn detection is disabled, and only the confirmed result for the committed audio reaches the session. Each destination retains its recognition credential namespace across method changes. Adapters refuse redirects and oversized inputs or responses and never save raw audio. Session Records carry the actual recognition provider and model while legacy serialized text fields remain compatible. Provider selection is recorded in [ADR-0010](adr/0010-select-speech-recognition-providers.md); recognition timing and final-only delivery are recorded in [ADR-0011](adr/0011-select-the-speech-recognition-method.md).
...
The main window is the only settings surface; the app menu's Settings command opens its Settings tab. It replaces writable controls while erasure is running. A failed erasure routes to a guarded recovery destination that replaces ordinary settings so its reason and retry action remain reachable without reopening writable controls.
