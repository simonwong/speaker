...
package enum SpeakerDiagnosticReport {
    ...
    package static func make(from snapshot: Snapshot) -> String {
        var lines = [
            ...
            "recognitionMethod: \(snapshot.recognitionProfile.method.rawValue)",
            "recognitionModel: \(SpeechRecognitionProviderCatalog.modelIDs(for: snapshot.recognitionProfile.provider, method: snapshot.recognitionProfile.method).contains(snapshot.recognitionProfile.model) ? snapshot.recognitionProfile.model : "custom")",
