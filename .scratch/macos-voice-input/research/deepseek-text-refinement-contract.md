...
- Speaker 不设置基于应用内经过时间的业务超时。等待状态只由 provider/URL Loading System 返回的明确结果、明确系统网络超时或用户取消结束，不能仅凭“等了多久”猜测服务失败。
...
- 429、500、503 或网络失败：当前不做应用层自动重试；立即保留豆包结果并记录明确分类，避免一次语音输入产生重复付费请求或不可预测延迟。
- `length`、`content_filter`、`tool_calls`、`insufficient_system_resource`、空 JSON 或解析失败：不把结果送达，不自动重试，直接保留豆包结果。
...
3. JSON Output 偶发空内容的频率，以及中国大陆实际网络中的等待分布和系统网络错误形态；
