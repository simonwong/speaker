...
13. As a user, I want empty, silent, or sub-300 ms recordings rejected locally, so that no unnecessary request is sent.
...
- The audio adapter records 16 kHz, 16-bit, mono PCM WAV. Recordings shorter than 300 ms or without useful level are rejected, and a 60-second watchdog releases the session. Temporary audio is deleted on every terminal path.
...
- Delivery uses a conservative ordered set of internally verified adapters. Direct Accessibility insertion is attempted only when required text attributes are writable, the target remains valid and non-secure, and the captured version evidence has not detected concurrent change. A PID-targeted Unicode event path is attempted only when the same element is still focused. The App never restores focus, simulates Command-V, rewrites an entire rich document value, or optimistically reports success.
...
- The source distribution includes a reproducible command-line build and test workflow. On the current machine it selects the compatible macOS 15.4 SDK and a writable module cache when the default Command Line Tools SDK/compiler pair is incompatible. Full Xcode is recommended for interactive UI debugging but is not a prerequisite for command-line verification.
...
- The current machine has macOS 26.5, Apple Silicon, Swift 6.2.4, and Command Line Tools. The default macOS 26.2 SDK is patch-incompatible with that compiler, while the installed macOS 15.4 SDK successfully compiles AppKit, ApplicationServices, and AVFoundation when explicitly selected.
