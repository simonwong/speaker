...
- 默认“语义顺滑”只使用豆包返回的文字，不调用 DeepSeek。
- 只有用户明确选择“精简清理”“完整重写”或自定义模式时，Speaker 才把豆包返回的文字、所选整理规则和本次按下快捷键时的个人词库词条发送到用户配置的 DeepSeek 账号；词条只作为纯文本数据，用于把听错的片段纠正回用户的拼写。
- Speaker 不向 DeepSeek 发送目标应用信息或会话历史。
