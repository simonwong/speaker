...
- 默认“语义顺滑”只使用豆包返回的文字，不调用文字整理服务。
- 只有用户明确选择“精简清理”“完整重写”或自定义模式时，Speaker 才把豆包返回的文字、所选整理规则和本次按下快捷键时的个人词库词条发送到用户所选的文字整理服务（DeepSeek、OpenAI、Kimi、GLM 或自定义 API）；词条只作为纯文本数据，用于把听错的片段纠正回用户的拼写。
- Speaker 不向 文字整理服务 发送目标应用信息或会话历史。
- Speaker 不把音频发送给 文字整理服务。
- 豆包与 文字整理服务 请求使用临时网络会话，不持久化 Cookie、HTTP 缓存或系统 URL 凭据。

- 自定义 API 只接受 HTTPS 基础地址；更换地址后必须重新保存 Key。文字整理请求拒绝重定向，不自动切换服务。
- “检查连接”仅在用户点击后发送合成测试文字，可能产生服务费用；不发送录音或历史。

## 本地保存的数据

会话历史保存在当前 macOS 用户的 `~/Library/Application Support/Speaker/history.sqlite3`，可能包含：

- 豆包转录、可选的 文字整理服务 结果和最终文字；
- 处理阶段、取消发生的阶段、耗时和服务请求 ID；
- 当次整理服务、模型 ID、整理规则及个人词库快照；
...
- 各文字整理服务的 Key 独立保存；删除一家不会删除其他服务的 Key。
