...
- **自选语音识别** — 豆包边录边传；OpenAI、千问在录音结束后识别整段音频。
...
3. 在 **语音识别** 中选择豆包、OpenAI 或阿里千问并保存对应 Key。豆包需选择已开通资源并执行 **检查连接**；千问需选择与 Key 对应的区域。语音识别和文字整理分别保存 Key。
...
1. Speaker 在内存中把麦克风输入转换为 16 kHz、16-bit、单声道 PCM，只发送给选中的语音识别服务。豆包分片发送；OpenAI、千问在录音结束后提交内存中的 WAV。
...
OpenAI 单次录音最多 5 分钟；千问为满足编码后的请求大小限制，单次最多 3 分钟。音频仅留在内存中。保存 OpenAI 或千问的 Key 不会发送付费探测，首次实际识别才验证账号和模型可用性。
