...
Speaker 常驻菜单栏，默认使用 `Fn` 快捷键。你可以按住说话、松开结束，也可以短按一次开始、再次短按结束。Speaker 可选豆包、OpenAI 或阿里千问识别语音，也可独立选择文字整理服务处理已确认的文本，并把最终结果送到录音结束时聚焦的输入位置。
...
- **自选语音识别** — 支持豆包、OpenAI、千问；OpenAI、千问可选流式识别（边录边传）或整段识别（录完上传）。
- **可选文本整理** — 默认顺滑直接采用所选识别服务的结果；精简清理、完整重写和自定义模式使用独立选择的文字整理服务与 Key，并且只发送文字，不发送音频。
...
1. Speaker 在内存中把麦克风输入转换为 16 kHz、16-bit、单声道 PCM，只发送给选中的语音识别服务。豆包分片发送；OpenAI、千问按用户选择边录边传，或在录音结束并通过本机校验后提交内存中的 WAV。OpenAI 流式识别会在内存中重采样为 24 kHz。
2. 松开快捷键时，Speaker 冻结当前输入目标，并等待识别服务返回最终阶段结果。
3. 默认顺滑直接采用识别结果；其他整理模式可以把已确认文字和所选指令发送给独立选择的文字整理服务。
4. 真正送达前，Speaker 会再次验证原输入目标。无法确认、已经变化、已经关闭或属于安全输入框时，结果会转为待复制状态。

音频只会发送给选中的语音识别服务，不会发送给文字整理服务，也不会作为普通应用数据持久化保存。

两种识别方式均保留 Speaker 的录音上限：OpenAI 单次最多 5 分钟，千问单次最多 3 分钟。已有设置保留原识别方式。流式音频会在最终本机校验前发送；取消会停止继续发送，但无法撤回服务商已接收的音频。两种方式都只整理和送达最终确认文字，不承诺某一种必然更准确。音频仅留在内存中。保存 OpenAI 或千问的 Key 不会发送付费探测，首次实际识别才验证账号和模型可用性。
